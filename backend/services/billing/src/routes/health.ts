...
healthRoutes.get('/',
  async (req: Request, res: Response) => {
    try {
      ...
      res.json(createResponse(health, 'Billing service is healthy'));
...
healthRoutes.get('/detailed',
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      res.status(statusCode).json(createResponse(health, `Billing service is ${overallStatus}`));
...
async function checkRedisHealth(): Promise<{
  ...
  try {
    const testKey = `billing_health_check_${Date.now()}`;
...
async function checkStripeHealth(): Promise<{
  ...
  try {
    // Simple check - just verify we can import Stripe
    ...
    return {
      ...
      responseTime,
      error: stripeConfigured ? undefined : 'Stripe not configured',
    };
