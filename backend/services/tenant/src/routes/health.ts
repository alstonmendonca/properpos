...
healthRoutes.get('/detailed', async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    ...
    try {
      ...
      const [organizationCount, activeOrganizations] = await Promise.all([
    ...
    res.json(createResponse(responseData, 'Tenant service is healthy'));
    return;
...
healthRoutes.get('/readiness', async (req: Request, res: Response) => {
  try {
    ...
    await getPlatformDatabase().collection('organizations').findOne({}, { limit: 1 });
 ...
 *     description: Returns basic performance metrics and tenant statistics
 */
healthRoutes.get('/metrics', async (req: Request, res: Response) => {
  try {
    ...
    let tenantMetrics = {};
    try {
      const [totalOrgs, activeOrgs, trialOrgs, paidOrgs] = await Promise.all([
        getPlatformDatabase().collection('organizations').countDocuments({}),
        getPlatformDatabase().collection('organizations').countDocuments({ isActive: true }),
        getPlatformDatabase().collection('organizations').countDocuments({ 'subscription.status': 'trial' }),
        getPlatformDatabase().collection('organizations').countDocuments({ 'subscription.status': 'active' }),
      ]);
      ...
      logger.warn('Failed to get tenant metrics for health endpoint', {
        error: error instanceof Error ? error.message : 'Unknown error',
      });
    }
...
export const performHealthCheck = async (): Promise<{ healthy: boolean; details: any }> => {
  ...
  try {
    ...
    await getPlatformDatabase().admin().ping();
    checks.platformDatabase = { status: 'healthy' };
  } catch (error) {
    checks.platformDatabase = {
      status: 'unhealthy',
      error: error instanceof Error ? error.message : 'Unknown error'
