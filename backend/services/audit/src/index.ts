...
// Create Express app
const app = express();
const PORT = process.env.PORT || process.env.AUDIT_PORT || 3009;
...
app.use((error: any, req: express.Request, res: express.Response, next: express.NextFunction) => {
  ...
  if (apiError.status >= 500 || !isOperationalError(apiError)) {
    logger.error('Audit service error', { error: apiError.message, ...errorContext });
  } else {
...
// Retention service for scheduled cleanup
let retentionService: RetentionService | null = null;

// Graceful shutdown
const gracefulShutdown = async (signal: string) => {
  logger.info(`Audit service received ${signal}. Starting graceful shutdown...`);
...
process.on('uncaughtException', (error: Error) => {
  logger.error('Uncaught Exception in Audit Service:', error);
  gracefulShutdown('uncaughtException');
});

process.on('unhandledRejection', (reason: any, promise: Promise<any>) => {
  logger.error('Unhandled Rejection in Audit Service:', promise, 'reason:', reason);
...
const startServer = async () => {
  try {
    ...
    // Initialize retention service (runs daily cleanup)
    retentionService = new RetentionService();
    retentionService.start();

    // Start HTTP server
    const server = app.listen(PORT, () => {
      logger.info(`📋 ProperPOS Audit Service started successfully`, {
        port: PORT,
        environment: process.env.NODE_ENV || 'development',
