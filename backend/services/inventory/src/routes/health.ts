// Health monitoring routes for Inventory Service
...
healthRoutes.get('/',
  async (req: Request, res: Response) => {
    try {
      ...
      res.json(createResponse(health, 'Inventory service is healthy'));
...
healthRoutes.get('/detailed',
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      res.status(statusCode).json(createResponse(health, `Inventory service is ${overallStatus}`));
...
async function checkRedisHealth(): Promise<{
  ...
  try {
    const testKey = `inventory_health_check_${Date.now()}`;
